...
The first successful TOTP or passkey registration creates eight recovery keys. Users can add more keys in batches, up to 32 active keys, or replace all existing keys. Plaintext recovery keys are displayed only once after creation. Register multiple passkeys to avoid relying on a single device.
